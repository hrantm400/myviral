...
export async function createSandwichVideo(
  ...
  if (subtitlePath && logoPath) {
    captionArgs = [
      ...
      "-i", logoPath,
      "-filter_complex",
      `[1:v]scale=120:-1[logo];[0:v][logo]overlay=W-w-30:30,ass='${subtitlePath.replace(/'/g, "\\'")}'[out]`,
      "-map", "[out]",
      "-map", "0:a",
      "-c:v", "libx264",
      "-preset", "medium",
    ...
    await execFileAsync("cp", [outputClearPath, outputCaptionPath]);
    return;
  }
